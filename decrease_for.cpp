...
void convolution_dec_for(const string& in_file){
    ...
    for(int i = 1; i < file_header.height-1; i++){
        for(int j = 1; j < file_header.width-1; j++){
            for(int c = 0; c< 3; c++){
                piccel2[(j + file_header.width* i)*3 + c] -= (int16_t)piccel[(j + file_header.width* (i-1))*3 + c];
                piccel2[(j + file_header.width* i)*3 + c] -= (int16_t)piccel[(j + file_header.width* (i+1))*3 + c];
            }
        }
    }
    ...
    for(int i = 1; i < file_header.height-1; i++){
        for(int j = 1; j < file_header.width-1; j++){
            for(int c = 0; c< 3; c++){
                piccel2[(j + file_header.width* i)*3 + c] -= (int16_t)piccel[((j+1) + file_header.width* (i))*3 + c];
                piccel2[(j + file_header.width* i)*3 + c] -= (int16_t)piccel[((j-1) + file_header.width* (i))*3 + c];
            }
        }
    }
